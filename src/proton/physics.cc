...
void PhysicsBody::bindShape(Shape *shape)
{
    /*if (std::find(physicsBodies.begin(), physicsBodies.end(), physicsBodies) != physicsBodies.end()) {
        ...
        Log("Unexistent physics body. You probably haven't added this body to a scene using addBody(Proton::PhysicsBody
    body).");
    }*/
}

PhysicsBody::~PhysicsBody() { this->bodyId = b2_nullBodyId; }

Shape *PhysicsBody::getUsedShape() const { return this->usedShape; }

float PhysicsBody::getPosX() const { return this->posX; }

float PhysicsBody::getPosY() const { return this->posY; }

b2BodyId PhysicsBody::getBody() const { return this->bodyId; }
} // namespace Proton
